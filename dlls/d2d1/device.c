...
#define GLYPH_RUN_CACHE_MAX 16384

struct glyph_run_cache_entry
{
    IDWriteFontFace *fontface;
    UINT16 glyph_indices[32];
    UINT32 glyph_count;
    FLOAT emSize;
    BOOL sideways;
    UINT32 bidi;
...
static struct glyph_run_cache_entry glyph_run_cache[GLYPH_RUN_CACHE_MAX];
static unsigned glyph_run_cache_pos = 0;

static unsigned glyph_run_cache_limit = 2048;
static BOOL glyph_run_cache_enabled = TRUE;
static BOOL glyph_run_cache_initialized = FALSE;
...
static void glyph_run_cache_init(void)
{
    if (glyph_run_cache_initialized)
        return;

    glyph_run_cache_initialized = TRUE;

    const char *env;

    env = getenv("WINE_D2D_GLYPH_RUN_CACHE");
    if (env)
        glyph_run_cache_enabled = atoi(env);

    env = getenv("WINE_D2D_GLYPH_RUN_CACHE_SIZE");
    if (env)
    {
        unsigned v = atoi(env);
        if (v > 0 && v <= GLYPH_RUN_CACHE_MAX)
            glyph_run_cache_limit = v;
    }

    TRACE("GlyphRunCache enabled=%d size=%u\n", glyph_run_cache_enabled, glyph_run_cache_limit);
}
...
static ID2D1PathGeometry *glyph_run_cache_lookup(const DWRITE_GLYPH_RUN *glyph_run)
{
    for (unsigned i = 0; i < glyph_run_cache_limit; i++)
    {
        struct glyph_run_cache_entry *e = &glyph_run_cache[i];

        if (!e->valid)
            continue;

        if (e->fontface != glyph_run->fontFace)
            continue;

        if (e->glyph_count != glyph_run->glyphCount)
            continue;

        if (e->emSize != glyph_run->fontEmSize)
            continue;

        if (e->sideways != glyph_run->isSideways)
            continue;

        if (e->bidi != glyph_run->bidiLevel)
            continue;

        if (glyph_run->glyphCount > 32)
            continue;

        if (memcmp(e->glyph_indices, glyph_run->glyphIndices,
            sizeof(UINT16) * glyph_run->glyphCount) != 0)
            continue;

        ID2D1PathGeometry_AddRef(e->geometry);
        return e->geometry;
    }
...
static void glyph_run_cache_insert(const DWRITE_GLYPH_RUN *glyph_run, ID2D1PathGeometry *geometry)
{

    if (glyph_run->glyphCount > 32)
        return;

    struct glyph_run_cache_entry *e = &glyph_run_cache[glyph_run_cache_pos++ % glyph_run_cache_limit];

    if (e->valid && e->geometry)
        ID2D1PathGeometry_Release(e->geometry);

    e->fontface = glyph_run->fontFace;
    e->glyph_count = glyph_run->glyphCount;
    e->emSize = glyph_run->fontEmSize;
    e->sideways = glyph_run->isSideways;
    e->bidi = glyph_run->bidiLevel;

    memcpy(e->glyph_indices, glyph_run->glyphIndices,
           sizeof(UINT16) * glyph_run->glyphCount);

    e->geometry = geometry;
    ID2D1PathGeometry_AddRef(geometry);
...
static void d2d_device_context_draw_glyph_run_outline(struct d2d_device_context *context,
                                                      ...
                                                      ID2D1Brush *brush)
{
    glyph_run_cache_init();

    ID2D1PathGeometry *geometry = NULL;
    HRESULT hr;

    if (glyph_run_cache_enabled)
        geometry = glyph_run_cache_lookup(glyph_run);
    ...
    if (!geometry)
    {
        ...
        if (glyph_run_cache_enabled)
            glyph_run_cache_insert(glyph_run, geometry);
    }
