...
static unsigned long glyph_cache_hits = 0;
static unsigned long glyph_cache_misses = 0;
static unsigned long glyph_cache_total = 0;

/* ===== FINAL GLYPH CACHE ===== */

#define DEFAULT_CACHE_SIZE (8192*8)
#define ORIGIN_Q 64.0f

static int cache_initialized = 0;
static int cache_enabled = 0;
static unsigned int simple_cache_max = DEFAULT_CACHE_SIZE;

static inline INT32 origin_to_fixed(FLOAT v)
{
    return (INT32)lrintf(v * ORIGIN_Q);
}
...
static struct simple_cache_entry *simple_cache = NULL;
static unsigned int simple_cache_count = 0;
...
static void glyph_cache_init(void)
{
    ...
    env = getenv("WINE_D2D_GLYPH_RUN_CACHE");
    if (!env || atoi(env) == 0)
    {
        cache_enabled = 0;
        return;
    ...
    env = getenv("WINE_D2D_GLYPH_RUN_CACHE_SIZE");
    if (env)
    {
    ...
    simple_cache = calloc(simple_cache_max, sizeof(*simple_cache));

    printf("[glyph-cache] enabled size=%u\n", simple_cache_max);
}

static struct simple_cache_entry *
simple_cache_find(struct dwrite_fontface *ff,
                  UINT16 glyph,
                  FLOAT emsize,
                  UINT32 sims,
                  const D2D1_POINT_2F *origin)
{
    INT32 ox = origin_to_fixed(origin->x);
    INT32 oy = origin_to_fixed(origin->y);

    for (unsigned int i = 0; i < simple_cache_count; i++)
    {
        struct simple_cache_entry *e = &simple_cache[i];
...
static void simple_cache_store(struct dwrite_fontface *ff,
                               UINT16 glyph,
                               FLOAT emsize,
                               UINT32 sims,
                               const D2D1_POINT_2F *origin,
                               const struct dwrite_outline *src)
{
    struct simple_cache_entry *e;

    if (!cache_enabled)
        return;

    if (simple_cache_count >= simple_cache_max)
        return;

    e = &simple_cache[simple_cache_count++];
    memset(e, 0, sizeof(*e));
    ...
    e->origin_x = origin_to_fixed(origin->x);
    e->origin_y = origin_to_fixed(origin->y);

    e->outline.tags.count = src->tags.count;
    e->outline.tags.size  = src->tags.count;
    e->outline.tags.values =
    malloc(sizeof(*src->tags.values) * src->tags.count);

    e->outline.points.count = src->points.count;
    e->outline.points.size  = src->points.count;
    e->outline.points.values =
    malloc(sizeof(*src->points.values) * src->points.count);

    if (!e->outline.tags.values || !e->outline.points.values)
    {
        free(e->outline.tags.values);
        free(e->outline.points.values);
        return;
    }

    memcpy(e->outline.tags.values,
           src->tags.values,
           sizeof(*src->tags.values) * src->tags.count);

    memcpy(e->outline.points.values,
           src->points.values,
           sizeof(*src->points.values) * src->points.count);

    for (unsigned int i = 0; i < e->outline.points.count; i++)
    {
        e->outline.points.values[i].x += origin->x;
        e->outline.points.values[i].y += origin->y;
    }
                                                        ...
                                                        UINT32 count, BOOL is_sideways, BOOL is_rtl, IDWriteGeometrySink *sink)
{
    ...
    if (FAILED(hr = compute_glyph_origins(&run,
    ...
    for (i = 0; i < count; ++i)
    {
        struct simple_cache_entry *cached = NULL;

        if (cache_enabled)
        {
            glyph_cache_total++;

            cached = simple_cache_find(
                fontface,
                glyphs[i],
                emSize,
                fontface->simulations,
                &origins[i]);
            ...
            double hitrate = 0.0;
            hitrate = (100.0 * glyph_cache_hits) / glyph_cache_total;

         //   printf("[glyph-cache] size=%u total=%lu hits=%lu misses=%lu hitrate=%.2f%%\n",
         //          simple_cache_count,
         //          glyph_cache_total,
          //         glyph_cache_hits,
          //         glyph_cache_misses,
        ...
        if (cached)
        {
            const struct dwrite_outline *use_outline = &cached->outline;

            for (j = 0, p = 0; j < use_outline->tags.count; ++j)
            {
                switch (use_outline->tags.values[j])
                {
                    case OUTLINE_BEGIN_FIGURE:
                        point = use_outline->points.values[p++];
                    ...
                    case OUTLINE_BEZIER:
                        segment.point1 = use_outline->points.values[p++];
                        segment.point2 = use_outline->points.values[p++];
                        segment.point3 = use_outline->points.values[p++];
        ...
        if (!(status = UNIX_CALL(get_glyph_outline, &params)))
        {
            dwrite_array_reserve(
                (void **)&outline.tags.values,
                                 &outline.tags.size,
                                 outline_size.tags.count,
                                 sizeof(*outline.tags.values));

            dwrite_array_reserve(
                (void **)&outline.points.values,
                                 &outline.points.size,
                                 outline_size.points.count,
